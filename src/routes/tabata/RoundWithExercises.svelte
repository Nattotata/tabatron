<script lang="ts">
  ...
  let selectedTag = Array.from(
    { length: amountOfTagsAndCancel },
    () => []
  );
...
<div class="ml-4 mt-4">
  ...
  <p>
    <strong>Počet cviků:</strong>
    {round.exercisesAmount}
  </p>
    ...
    {#each round.exercises as exercise, exerIndex (exerIndex)}
      <dl class="list-dl m-2">
        <dt class="flex flex-row">
          <span
            class="badge-icon p-4 variant-soft-primary mr-2"
            >{exerIndex + 1}</span
          >
          <span class="mx-2"
            ><h4 class="h4">
        ...
        <dd class="ml-10 mt-10">
          {#each exercise.tags as tag}
            <span class="m-1 chip variant-filled"
              >{tag}</span
            >
          {/each}
        </dd>
        ...
        <dd class="mx-8">
          <select
            ...
            bind:value={handPickedExercise[index][
              exerIndex
            ]}
            on:change={() => {
              if (
                handPickedExercise[index][exerIndex] ===
                "cancel"
              )
                return;
              ...
              templateWithExercises.update((rounds) => {
                rounds[index].exercises[exerIndex] =
                  newExercise;
                return rounds;
              });
            }}
          >
            ...
            {#each findExercisesByTags( { tags: exercise.tags, allExercises } ) as similarExercise}
              <option
                id="handPickedExercise"
                value={similarExercise.name}
              >
        ...
        <dt class="ml-10">
          Vyměň za náhodný cvik se zvoleným štítkem:
        </dt>
        <dd class="flex flex-row mx-8 gap-2">
          <span>
            <select
              class="select"
              bind:value={selectedTag[index][exerIndex]}
            >
              <option value="cancel" selected
                >Zrušit výběr</option
              >
          ...
          <button
            ...
            on:click={() => {
              if (
                selectedTag[index][exerIndex] === "cancel"
              )
                return;
